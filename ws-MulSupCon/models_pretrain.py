...
class Encoder(nn.Module):
    def __init__(
        ...
        if backbone == 'DenseNet121':
            self.backbone = torchvision.models.densenet121(weights="DenseNet121_Weights.IMAGENET1K_V1")
        ...
        with torch.no_grad():
            ...
            rep_dim = self.backbone(x).flatten(1).shape[-1]
...
class PretrainModel(PretrainBase):
    def __init__(
        ...
        output_func_name: str = 'MulSupCon',
        cap_Q: int = 2048,
    ...
    def forward(self, im_q, im_k, labels):
        ...
        # q and k inner product, dim = B*B
        type = ['MulSupCon', 'MulSupCon_sm', 'MulSupCon_smw', 'SimDiss', 'any', 'IoU', 'MulSupCon_iw']
        # if name in the type list then return the output_func
        if self.output_func_name in type: # no splitting into single and multi
        ...
        elif self.output_func_name == 'SoftCon':
            batch_score = torch.einsum(
                'bd,dk->bk', [q, all_k.transpose(0, 1)])
            assert torch.equal(labels, all_labels)
            return self.output_func(
                labels, all_labels, batch_score)

        # todo
        elif self.output_func_name == 'MulSupCon_iwa' or self.output_func_name == 'MulSupCon_iwas' \
            or self.output_func_name == 'MulSupCon_iwasn' or self.output_func_name == 'MulSupCon_iwasm' or self.output_func_name == 'MulSupCon_iwash':
            # need to input dataset name
            batch_score = torch.einsum(
                'bd,dk->bk', [q, all_k.transpose(0, 1)])
            if not self.QueueFull[0]:
                # return self.output_func(
                #     labels, all_labels, batch_score,  dataset=self.args.dataset, wtype = self.args.output_func_w, neg_weight_s = self.args.neg_weight_s, 
                #     neg_weight_m = self.args.neg_weight_m, mwtype = self.args.output_func_m)

                return self.output_func(
                    labels, all_labels, batch_score, args = self.args)
            ...
            # return self.output_func(
            #     labels, torch.cat([all_labels, queue_labels], dim=0), scores, dataset=self.args.dataset, wtype = self.args.output_func_w, 
            #     neg_weight_s = self.args.neg_weight_s, neg_weight_m = self.args.neg_weight_m, mwtype = self.args.output_func_m)

            return self.output_func(
                    labels, torch.cat([all_labels, queue_labels], dim=0), scores, args = self.args)

        elif self.output_func_name == 'MulSupCon_s':
            q_s, _, ql_s, _ = self._split_single_multi(q, labels)
            k_s, _, kl_s, _ = self._split_single_multi(all_k, labels, ref=True)

            batch_score_s = torch.einsum(
                'bd,dk->bk', [q_s, k_s.transpose(0, 1)])

            if not self.QueueFull[0]:
                return self.output_func(
                    ql_s, kl_s,  batch_score_s, self.args.output_func_w)
            
            # queue
            queue_k_s, _, queue_l_s, _ = self._split_single_multi(queue_k, queue_labels, ref=True)
            queue_score_s = torch.einsum(
                'bd,dk->bk', [q_s, queue_k_s.transpose(0, 1)])
            scores_s = torch.cat([batch_score_s, queue_score_s], dim=1)
            all_labels_s = torch.cat([kl_s, queue_l_s], dim=0)

            return self.output_func(
                ql_s, all_labels_s, scores_s, self.args.output_func_w)
        
        elif self.output_func_name == 'MulSupCon_m':
            _, q_m, _, ql_m = self._split_single_multi(q, labels)
            _, k_m, _, kl_m = self._split_single_multi(all_k, labels, ref=True)

            batch_score_m = torch.einsum(
                'bd,dk->bk', [q_m, k_m.transpose(0, 1)])
            if not self.QueueFull[0]:
                return self.output_func(
                    ql_m, kl_m,  batch_score_m, self.args.output_func_w)
            
            # queue
            _, queue_k_m, _, queue_l_m = self._split_single_multi(queue_k, queue_labels, ref=True)
            queue_score_m = torch.einsum(
                'bd,dk->bk', [q_m, queue_k_m.transpose(0, 1)])
            scores_m = torch.cat([batch_score_m, queue_score_m], dim=1)
            all_labels_m = torch.cat([kl_m, queue_l_m], dim=0)

            return self.output_func(
                ql_m, all_labels_m, scores_m, self.args.output_func_w)
        
        else:
            q_s, q_m, ql_s, ql_m = self._split_single_multi(q, labels)
            k_s, k_m, kl_s, kl_m = self._split_single_multi(all_k, labels, ref=True)
            ...
            if not self.QueueFull[0]:
                return self.output_func(
                    [ql_s, ql_m], [kl_s, kl_m], [batch_score_s, batch_score_m], self.args.output_func_w)
            
            # queue
            queue_k_s, queue_k_m, queue_l_s, queue_l_m = self._split_single_multi(queue_k, queue_labels, ref=True)
            queue_score_s = torch.einsum(
                'bd,dk->bk', [q_s, queue_k_s.transpose(0, 1)])
            scores_s = torch.cat([batch_score_s, queue_score_s], dim=1)
            queue_score_m = torch.einsum(
            ...
            return self.output_func(
                [ql_s, ql_m], [all_labels_s, all_labels_m], [scores_s, scores_m], self.args.output_func_w)

    
    def _split_single_multi(self, feats, labels, ref = False):
        if not ref:
            feats_s = feats[labels.sum(1) == 1]
            feats_m = feats[labels.sum(1) > 1]
            labels_s = labels[labels.sum(1) == 1]
            labels_m = labels[labels.sum(1) > 1]
            return feats_s, feats_m, labels_s, labels_m
        else:
            feats_s = feats[labels.sum(1) <= 1]
            feats_m = feats[labels.sum(1) > 1]
            feats_mh = feats[labels.sum(1) == 0]
            feats_m = torch.cat([feats_m, feats_mh], dim=0)

            labels_s = labels[labels.sum(1) <= 1]
            labels_m = labels[labels.sum(1) > 1]
            labels_mh = labels[labels.sum(1) == 0]
            labels_m = torch.cat([labels_m, labels_mh], dim=0)
